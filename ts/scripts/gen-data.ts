 ...
 *
 * The same splitmix hash and the same drift recipe as the Python, Java, Go and
...
/** 64-bit wrap-around, which JavaScript's BigInt does not do on its own. */
const MASK = (1n << 64n) - 1n;

/** Writes both files in one pass. */
export function genData(rows: number, aPath: string, bPath: string, seed = 7): void {
  ...
  const money = (cents: number): string =>
    `${cents < 0 ? "-" : ""}${Math.floor(Math.abs(cents) / 100)}.${pad(Math.abs(cents) % 100, 2)}`;

  const row = (i: number, b: boolean): string => {
    // Money is carried in integer cents and the drift is applied to those
    // integers, never to a float, so every implementation of this generator
    // writes the same digits whatever its rounding rule.
    const bucket = hm(i, 0, 10000);
    let amountCents = hm(i, 21, 900000000) - 100000000;
    let balanceCents = hm(i, 31, 2000000000);
    ...
    if (b) {
      ...
      else if (bucket < CHG_STATUS + CHG_AMOUNT) amountCents += 1234;
      // +1%, rounded half up, in cents.
      else if (bucket < CHG_STATUS + CHG_AMOUNT + CHG_BALANCE) balanceCents = Math.floor((balanceCents * 101 + 50) / 100);
...
async function main(): Promise<number> {
  ...
  genData(rows, a, b, Number(values.seed));
  const dt = (performance.now() - t0) / 1000;
  console.log(`typescript: ${rows.toLocaleString("en-US")} rows x ${COLUMNS.length} columns in ${dt.toFixed(1)}s`);
