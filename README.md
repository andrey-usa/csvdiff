...
|---|---|---:|---:|---:|---:|---:|---:|
| **Zig AVX2** | **CSV, 3,509 MB** | **2.21s** | **4,528,575** | **7.7s** | 3.47x | 4,387 MB | **878 MB** |
| Rust AVX2 | CSV | 2.52s | 3,974,645 | 8.4s | 3.33x | 4,408 MB | 899 MB |
| Rust, engine only | CSV | 2.56s | 3,905,901 | 9.1s | **3.56x** | 4,385 MB | 877 MB |
| Zig | CSV | 2.67s | 3,751,410 | 9.3s | 3.48x | 4,395 MB | 887 MB |
| Rust | CSV | 3.06s | 3,263,576 | 10.3s | 3.37x | 4,408 MB | 899 MB |
| C++ AVX2 | CSV | 9.18s | 1,089,673 | 26.9s | 2.93x | 4,389 MB | 880 MB |
| C++ SWAR | CSV | 9.73s | 1,027,635 | 28.8s | 2.96x | 4,390 MB | 881 MB |
| C++ | CSV | 9.83s | 1,017,661 | 29.1s | 2.96x | 4,388 MB | 879 MB |
| **Rust, engine only** | **Parquet, 1,535 MB** | **2.36s** | **4,236,483** | **8.1s** | 3.41x | 2,863 MB | 1,329 MB |
| Zig | Parquet | 2.66s | 3,758,951 | 9.4s | **3.54x** | 2,756 MB | **1,221 MB** |
| Zig AVX2 | Parquet | 2.67s | 3,747,959 | 9.4s | 3.51x | **2,755 MB** | 1,221 MB |
| Rust | Parquet | 2.77s | 3,616,121 | 8.9s | 3.22x | 2,908 MB | 1,373 MB |
| Rust AVX2 | Parquet | 2.77s | 3,611,854 | 8.8s | 3.18x | 2,882 MB | 1,347 MB |
| C++ | Parquet | 3.02s | 3,311,269 | 9.6s | 3.18x | 2,852 MB | 1,317 MB |
...
[The run.](https://github.com/andrey-usa/csvdiff/actions/runs/34347601844) This
runner reported `vector units: avx2` and no `avx512bw`, so the 64-byte builds
skipped themselves and there is no AVX-512 row to read. The rows are sorted by
time rather than by port, because the fastest build in each format is no longer
the port's default one.
...
**Zig now finishes CSV in a bit over a quarter of the time the C++ port takes**,
which is not what this branch set out to find: it set out to see whether Rust and
Zig could *match* a C++ port that started three times ahead. As a ratio to the
C++ row, which is the only fair way to read across machines:

| CPU, as a fraction of the C++ port's | Four rounds ago | Last round | Now |
|---|---:|---:|---:|
| Zig on CSV | 0.90 | 0.62 | **0.32** |
| Rust on CSV | 0.94 | 0.69 | **0.35** |
| Zig on Parquet | 1.05 | 1.03 | **0.98** |
| Rust on Parquet | 1.15 | 1.03 | **0.93** |

The CSV column halved again in one round, and the round that did it is [what the
join was still parsing](#what-the-join-was-still-parsing). Parquet barely moved,
which is the control working: neither change in that round touches the columnar
path, and its rows drifted only as far as the machine did.
...
strings, sorted, gzipped and embedded. Those two numbers have not moved in two
runs, and should not: the row sections are capped at fifty thousand, so the
report is the one part of the run whose cost does not grow with the file, and
nothing in the last round went near it. As the engine gets quicker the report
becomes a larger share of the Rust row -- a sixth of it now, where it was an
eighth. [The gzip going parallel](#gzip-was-not-the-serial-tail-it-was-described-as)
is what took it to 0.50s from 0.76s.

**Parquet is 3.3x quicker than CSV in the same port and holds 1.5 GB less**
(the C++ port: 9.83s and 4,388 MB against 3.02s and 2,852 MB), where the first
...
the CSV rows use, which meant every value had to exist somewhere at once. The
columnar path never builds a row — it joins on the key columns and compares whole
columns as `int32` ids, one column at a time — so 8.33s and 8,415 MB became
2.67s and 2,834 MB on the same file, on the machine that measured both. The
row-materialising reader survives only
as the fallback for `a.parquet` against `b.csv`, which has no column to compare
a byte stream against.

**The cores column went down this round, and that is the point.** Four rounds ago
every CSV row sat between 2.80x and 3.01x, and the rounds that followed pushed
Rust and Zig to 3.69x and 3.74x by finding work that was sitting on one thread.
They now read 3.37x and 3.48x. Nothing regressed: [the last
round](#what-the-join-was-still-parsing) deleted parallel work rather than
spreading it, and deleting work a thread was doing lowers the ratio while the
clock improves. Cores-busy is a diagnosis, not a target -- it says whether a slow
row is idle or merely working too hard, and the answer for these two is now
neither. The untouched C++ rows sit where they always did, a little under three.

* **Rust holds 877 MB above the input where it used to hold 1,193 MB.** That is
  the rehash fix: the index was sized for a two-thirds load and then doubled
  itself at one half, so it spent the run in a table twice the size it was
  designed for.
* **Zig on Parquet was the least parallel row in this table and is now the most**
  — 2.66x became 3.54x. The cause this file used to name, serial dictionary
  interning, was never measured and is not real: [what actually capped
  it](#what-the-columnar-path-was-waiting-on). Which of the two ports leads that
  format on the clock has now changed hands three runs running, by margins of a
  tenth of a second; neither of the last two rounds went near the columnar path,
  so that is the machine talking and not a result.

**The 32-byte scanner now wins CSV outright in both ports** — Zig 2.21s against
2.67s and Rust 2.52s against 3.06s, both against their own SWAR default, and the
C++ port's AVX2 build beats its own SWAR by 6%. That is a reversal of the last
run, where the widths finished level, and the reason is the round in between: the
join stopped parsing rows it did not need, so scanning is a larger share of what
is left and the width that scans faster wins by more. A saving that looks like
nothing beside expensive work becomes visible once the expensive work is gone,
which is the general form of this and not a fact about AVX2.

There is no AVX-512 row to compare against: this runner has none, and the 64-byte
builds skipped themselves. Three machines have now given three answers about that
width — one put Zig's AVX-512 clearly first, one put it level with AVX2, and a
development container puts it 15% behind SWAR — and [what varies is the CPU
generation and whether anything downstream uses the bytes the width forced you to
read](#avx-512-measured-on-two-machines-that-disagree).

**AVX2 buys nothing on the Parquet path, in either port that has it** — Zig
2.66s against 2.67s, Rust 2.77s against 2.77s, both inside the noise, and the
same verdict this table has returned three runs running. The
...
### What the join was waiting on

The round after [What the CPU column found](#what-the-cpu-column-found), and a
different kind of hunt: every gap below was found by a phase timer rather than by
reading code, because in each case the guess made before measuring was wrong.

All three ports now print a phase breakdown on stderr under `CSVDIFF_PHASES`,
which is how these were found — the cores-busy column says *that* something is
serial and never which thing:

```
$ CSVDIFF_PHASES=1 csvdiff compare a.csv b.csv -k account_id,txn_id
  A sweep (parallel)           1.712s
  B sweep (parallel)           1.735s
  A index insert (serial)      0.706s
  B index insert (serial)      0.713s
    join chunks (par)          3.799s
  join                         4.061s
```

**The join was waiting on the side that looked cheap.** Both the Rust and the Zig
port gave B's side of the join a thread of its own and split A's over the rest,
for a reason the comment spelled out: A looks each of its keys up in B, reads both
rows and compares every column, where B only asks whether each of its keys exists
in A. Timing each task said the opposite. At four million rows B's single thread
took 3.105s while A's three ranges took 1.570s, 1.607s and 1.631s: the join spent
half its time on one core with three idle. A key does cost B less than it costs A
— about two thirds — but B has just as many of them, and no ratio like that makes
"one thread" the right unit for a side. Both sides are now chunks of 65,536 keys
in one queue that every thread pulls from, and the chunk count comes from the row
count rather than the thread count, so which rows a capped section keeps does not
depend on `--threads`.

**The index was rehashing at a load factor its own sizing contradicted.** `build`
sizes the table for a two-thirds load; `insert` grew it at one half. A file whose
keys are nearly all distinct crosses that on the way in and doubles a table sized
precisely so it would not have to. Four million rows squeaked under the threshold,
which is why it took the ten-million set to show it. The fix is one comparison,
and the two ways of making the two numbers agree are not close (ten million rows
on the four-core development container, phase timings, so these are not the Set D
runner's numbers):

| 10M rows | insert | join | total |
|---|---:|---:|---:|
| as it was | 7.07s | 3.80s | 13.47s |
| size for a half load | 4.39s | 4.68s | 11.49s |
| **rehash at two thirds** | **3.17s** | 4.39s | **9.71s** |

Sizing for a half load removes the rehash too, but the table it leaves is twice
as big, and at ten million keys the misses that costs outweigh the probes it
saves — it loses the insert by 27% *and* the join by 6% to the denser table.
Locality beat probe count, which is the opposite of the way load factors are
usually argued.

**A calloc was costing half the insert.** With the ports finally instrumented the
same way, Rust's insert was twice Zig's — 1.37s against 0.71s — for what is the
same loop over the same table. It was not the loop. `vec![EMPTY_SLOT; cap]`
compiles to a calloc, and a fresh anonymous mapping is one shared page of zeroes
until something writes to it; nothing reads the table before the inserts start, so
every one of its thirty-odd thousand pages was first touched by a *random* probe.
The Zig port allocates and memsets, which faults the same pages in ascending
order, and the kernel is far better at that. Writing the zeroes in Rust too closed
the whole gap.

**Every probe is now started thirty-two rows early.** The insert walks its own
file's hashes and the join walks the other file's keys, and both then probe a
table of tens of megabytes: a serial chain of misses, each waiting on an address
that was known long before the load was issued. Prefetching that far ahead is
worth 24% of the insert and 15% of the join, and the distance is not delicate —
eight, sixteen and thirty-two were within noise of each other, and four was not
quite enough.

**And every matched row was being parsed twice.** `lookup` finds a candidate,
parses that row into the caller's scratch and compares the key columns against
it — and then the join's very next line parsed the same row again into a second
buffer. The scratch already holds exactly those fields on a hit. Both ports had
...
into libc's `memcmp`, which callgrind says costs it 843M instructions against the
Zig port's 343M for the same comparisons — replacing it with two overlapping
...
### What the join was still parsing

The phase timer said the join was 43% of a one-million-row run and every other
phase was small. The obvious reading is that comparing eighteen columns of ten
million matched rows is expensive. It is not, and the way to find that out is to
stop doing it: a build that parses both rows and then skips the comparison
entirely runs the phase in **0.245s**, against **0.245s** for the build that
compares. Not close — the same number, three runs each.

So the join never was comparing. It was *parsing the rows it compared*, and two
rounds came out of that.

**The direction that only asks whether a key exists was reading twenty columns.**
The join runs both ways: A's direction reads its row, finds the mate and compares
every column against it, while B's asks one question — is this key in A? — and
throws away the row it matched against. `RowParser` already stops at the last
column it was asked for, and both ports already carried a parser built from the
key columns alone, because [the sweep](#what-the-join-was-waiting-on) took that
saving a round earlier. The join simply never used it. Giving `lookup` a `Want`,
so the discarding side parses two fields for its own row and two for the
candidate rather than forty, is **-8.5% in Rust and -6.5% in Zig** on the clock,
and moves the join phase by a fifth in both — which is what removing the same
work from the same place in two ports should look like.

**Then stop parsing the mate at all.** If A's row and B's row carry the same
bytes as far as the last column either file wants, then every column in between
is the same bytes too, and no parse can find a difference that byte equality has
already ruled out. The pair is unchanged and the mate is never read. On this
payload 94% of matched pairs are unchanged, so 94% of them collapse to one
comparison of about a hundred and fifty bytes: **-11.9% in Rust and -20.6% in
Zig**, on top of the round above.

That shortcut is only sound between files of the same shape — the same delimiter
and the same column-to-slot mapping — because two headers in different orders can
carry identical bytes and mean entirely different things. It is settled once, per
comparison, before the join starts. JSON never qualifies, having no such prefix,
and neither does a pair with a columnar side, which has no row bytes to compare.

Two guards make a run of bytes a whole number of columns rather than a truncation
of one, and there is a test for each that fails when the guard is removed:

- **The run must end on a field boundary in B**, or `12,3` matches a row that
  merely opens `12,34`. Deleting this check fails five tests.
- **And on a boundary in A**, or a run ending inside a quoted field — whose end
  is its closing quote, not a delimiter — matches a row that never closed one.
  This one only bites on malformed input, which is why its test holds the counts
  to the reference engine rather than the rendered values: the two already
  disagree about the trailing newline of an unterminated field, which is older
  than this and a separate question.

At ten million rows the pair of rounds is worth more than at one million, because
the join is a larger share of the run there while the report stays capped: Rust's
CSV row went **4.62s to 3.06s** and Zig's **4.36s to 2.67s** across them, on two
runners of the same class. The C++ rows moved +10% on unchanged source over the
same interval and every Parquet row moved with them, which is how much of that is
the machine and how much is the code.
...
| Build | Scanner | 10M CSV | CPU | Cores | vs SWAR at 10M | at 1M |
|---|---|---:|---:|---:|---:|---:|
| Zig | AVX2, 32 bytes | **7.62s** | 21.0s | 2.76x | **4.5%** | 12.9% |
| Zig | SWAR, 8 bytes | 7.98s | 21.9s | 2.75x | — | — |
| C++ | AVX2, 32 bytes | **8.48s** | 26.0s | 3.07x | **3.3%** | — |
| C++ | SWAR, 8 bytes | 8.77s | 27.1s | 3.09x | — | — |
| Rust | AVX2, 32 bytes | **8.89s** | 25.1s | 2.83x | **3.7%** | 15.0% |
| Rust | SWAR, 8 bytes | 9.23s | 25.7s | 2.79x | — | — |

**All three gain, and all three gain far less than the same builds do at a
million rows.** The last column is the same binaries on the same recipe, one
tenth the size: 12.9% and 15.0% become 4.5% and 3.7%. Nothing about the scanner
changed between those two columns — the working set did. At ten million the scan
is waiting on memory it cannot cache, so a wider register mostly waits faster;
at a million it fits, and the register width shows.

**So a scanner benchmark measures the memory system as much as the scanner.**
That is the same finding as the one this section opens with, arrived at from the
other side: the first table concluded SIMD loses to SWAR, and it was measuring a
byte-at-a-time key hash that hid the scan; this one says the margin depends on
the scale you quote it at. A scanner number without its scale attached is not a
number.

**What the vector register buys is less work, not better overlap.** Every port
keeps its cores-busy ratio and spends less CPU — Zig 21.9s to 21.0s, C++ 27.1s
to 26.0s, Rust 25.7s to 25.1s.
...
#### AVX-512, measured on two machines that disagree

The 64-byte builds had never run: no hosted runner this project had landed on
carried `avx512bw`, so they skipped themselves by name. Two things then happened
within an hour of each other, and the second overturned the first.

**A development container had AVX-512, and there the 64-byte scanner lost
badly.** A 4-core Intel Xeon at 2.80GHz, median of seven runs at a million and
five at four million, interleaved:
...
That was written up here as a flat negative result. It was wrong, and the way it
was wrong is worth more than the result would have been.

**The next benchmark run landed on an Intel Xeon Platinum 8573C — the first
hosted runner with `avx512bw` — and there Zig's 64-byte scanner is the fastest
build in the whole table.** Same source, same flags, ten million rows:

| Port | SWAR, 8 bytes | AVX2, 32 bytes | AVX-512, 64 bytes |
|---|---:|---:|---:|
| **Zig** | 7.72s | 7.23s | **7.07s** — *best in that table* |
| C++ | 7.93s | **7.38s** | 8.02s — *worse than its own SWAR* |

Those are [that run's](https://github.com/andrey-usa/csvdiff/actions/runs/34262369270)
numbers, not the current Set D's: no runner since has carried `avx512bw`, and
both ports have had a round of work in between. The comparison within the row is
what the section is about.

**So the register width is not the variable. Two other things are.**

*The CPU generation.* The container is an older Xeon, where issuing 512-bit
instructions drops the core clock for everything around them; the 8573C is
Emerald Rapids, where that penalty is largely gone. The same binary is 15%
slower than SWAR on one machine and 8% faster on the other. **An AVX-512 number
that does not name the CPU generation is not a result**, which is why the
container measurements are kept above rather than deleted — they are what that
hardware really does.

*Whether anything amortises the wide load.* On the good CPU, Zig's 64-byte
scanner wins while the C++ port's loses to its own SWAR. The difference between
them is the [delimiter cursor](#swar-and-how-it-compares-to-real-simd): fields
average 9.2 bytes, so one 64-byte load spans about seven of them, and the cursor
is what lets a single load serve all seven rather than being reloaded per field.
Zig has it; the C++ port's scanner does not. A wide register is worth nothing on
its own here — only once the cost of the load is spread across the fields it
covers, and on the older CPU not even then.

`make scanners` and `zig build -Dscan=64 -Dcpu=native` build these anywhere, and
the workflow runs them whenever a runner turns up with the instructions, which is
now sometimes rather than never.
...
prediction was that it should win by more there than it does on CSV.

**It does not. It wins nothing at all:** 4.06s against 4.11s at ten million rows,
inside the run-to-run noise, where the same build gains 11.4% on CSV. The
prediction was sound about the scan and wrong about its share of the run. Reading
the mask is not where the columnar path spends its time — the two gathers through
the dictionaries are, and they are bound by memory latency that no scanner width
touches. A 20% saving on 3% of a phase is not measurable, and the honest reading
is that this scanner was already fast enough to stop thinking about. The rows are
kept because a negative result measured is worth more than a prediction left
untested; `bench_formats_ports.py --matrix` prints them side by side.
...
counts on the same files. One machine, one sitting, ten million rows, when the
design first landed in all three:
...
so its key phases dominate its run.

**That last paragraph was a prediction, and it was worth about 3.5x rather than
the 2x it guessed.** The Zig port's column pass used every core while everything
before it ran on one: both indexes built one after the other, then both
directions of the join walked one after the other. Both phases are two
independent halves — the indexes share nothing, and once both exist the A
direction keeps the pairs it finds while the B direction only counts the keys
that find no mate, so it has no output to share — so each now runs on two
threads. With that and the word-at-a-time key hash, Zig's uncompressed Parquet
went **14.20s to 4.11s**, and the CSV numbers in this table moved as far again.
[Set D](#set-d--ten-million-rows-on-a-hosted-runner) is the current measurement
of all three; this table is kept as what the design was worth on the day it
landed, before any of the tuning that followed.
