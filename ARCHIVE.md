...
| **Proving the row unchanged where nothing is ignored behind it** | the column proof needs the last compared column's end, which is why this side is still parsed in full. Where that column is the row's last, the line ending is the end and a key-only parse would do -- worth 1.15x on a snapshot pair (5% of rows changed, nothing ignored) and **5-13% of CPU against the shape the project publishes**, where it does not apply. Splitting the loop so neither shape carries the other's branches did not recover it. Not landed: a workload introduced the same day does not get to cost the measured one |
